...
- Explain the role of Application Gateway in Azure networking.
...
In enterprise networking, Reverse proxy + WAF is a core design pattern that improves scale, resilience, and control. In Azure, the equivalent service is represented as a managed platform component with APIs, policy, and integration points that make the service programmable rather than purely manual.

## Azure explanation

Azure delivers Application Gateway as a cloud-native building block. The design focus shifts from physical device management to policy, routing, service integration, and lifecycle automation.

## Networking engineer translation

Think of Application Gateway as the Azure equivalent of Reverse Proxy + WAF. The difference is that the controls live in the cloud control plane and are applied through Azure-native constructs rather than standalone appliances.

## Architecture diagram

```mermaid
flowchart LR
  A[Enterprise Edge] --> B[Application Gateway]
  B --> C[Azure Resources]
  C --> D[Shared Services]
```
...
az network 12-appgateway show --resource-group rg-study --name example-12-appgateway
```
...
Get-AzApplication Gateway
```

## Bicep examples

```bicep
resource example '12-appgateway' = {
  name: 'example-12-appgateway'
...
Application Gateway is best understood as a managed Azure-native capability that replaces or augments traditional enterprise networking constructs.
