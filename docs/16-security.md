...
- Explain the role of Security in Azure networking.
...
In enterprise networking, Policy stack is a core design pattern that improves scale, resilience, and control. In Azure, the equivalent service is represented as a managed platform component with APIs, policy, and integration points that make the service programmable rather than purely manual.

## Azure explanation

Azure delivers Security as a cloud-native building block. The design focus shifts from physical device management to policy, routing, service integration, and lifecycle automation.

## Networking engineer translation

Think of Security as the Azure equivalent of Security stack. The difference is that the controls live in the cloud control plane and are applied through Azure-native constructs rather than standalone appliances.

## Architecture diagram

```mermaid
flowchart LR
  A[Enterprise Edge] --> B[Security]
  B --> C[Azure Resources]
  C --> D[Shared Services]
```
...
az network 16-security show --resource-group rg-study --name example-16-security
```
...
```bicep
resource example '16-security' = {
  name: 'example-16-security'
...
Security is best understood as a managed Azure-native capability that replaces or augments traditional enterprise networking constructs.
