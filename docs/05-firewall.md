...
- Explain the role of Azure Firewall in Azure networking.
...
In enterprise networking, Policy engine is a core design pattern that improves scale, resilience, and control. In Azure, the equivalent service is represented as a managed platform component with APIs, policy, and integration points that make the service programmable rather than purely manual.

## Azure explanation

Azure delivers Azure Firewall as a cloud-native building block. The design focus shifts from physical device management to policy, routing, service integration, and lifecycle automation.

## Networking engineer translation

Think of Azure Firewall as the Azure equivalent of FortiGate Policy Engine. The difference is that the controls live in the cloud control plane and are applied through Azure-native constructs rather than standalone appliances.

## Architecture diagram

```mermaid
flowchart LR
  A[Enterprise Edge] --> B[Azure Firewall]
  B --> C[Azure Resources]
  C --> D[Shared Services]
```
...
az network 05-firewall show --resource-group rg-study --name example-05-firewall
```
...
```bicep
resource example '05-firewall' = {
  name: 'example-05-firewall'
...
Azure Firewall is best understood as a managed Azure-native capability that replaces or augments traditional enterprise networking constructs.
